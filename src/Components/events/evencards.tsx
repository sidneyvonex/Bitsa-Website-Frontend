...
const getCategory = (event: Event) => {
  if (event.category) {
    ...
    return categoryStyles[key] ?? { badge: 'bg-blue-600 text-white', text: event.category };
  }

  const derived = Object.keys(categoryStyles).find((key) =>
    key !== 'default' && event.title.toLowerCase().includes(key),
  );

  if (derived && derived !== 'default') return categoryStyles[derived];
  ...
  return (
    <article className="bg-white rounded-2xl shadow-sm hover:shadow-lg transition-all border border-gray-100 overflow-hidden flex flex-col">
      ...
      <div className="flex-1 flex flex-col p-6 space-y-4">
        ...
        <Link
          to={`/events/${event.id}`}
          className={`mt-4 inline-flex items-center justify-center rounded-lg px-4 py-2 font-semibold text-sm transition-colors ${
            variant === 'past'
