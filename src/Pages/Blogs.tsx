...
const BlogCard = ({ blog }: { blog: Blog }) => {
  const authorName =
    [blog.author?.firstName, blog.author?.lastName].filter(Boolean).join(' ') || 'BITSA Editorial';
  ...
  return (
    <article className="group bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden flex flex-col h-full">
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        <p className="text-gray-600 text-sm line-clamp-3 flex-1">{blog.excerpt}</p>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {loadingLatest ? (
                ...
                latestBlogs.map((blog) => (
                  <Link key={blog.id} to={`/blogs/${blog.slug ?? blog.id}`} className="block h-full">
                    <BlogCard blog={blog} />
                  </Link>
                ))
              )}
